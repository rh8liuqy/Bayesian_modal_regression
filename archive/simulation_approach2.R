rm(list = ls())
# this is alternative approach about simulation study with the right-skewed noise.
# approach 2 uses the optimal bandwidth selection procedure proposed by Weixin Yao.
# However, this leads to unsatisfying coverage rate in the bootstrap procedure.
...
simu_MODLR <- function(seed) {
  ...
  # point estimation
  point_est <- MODLR(y = y, X = X, beta_init = beta_true, h = 1, iter_max = 1000, tol = 1e-8)
  
  # define residuals
  MODLR_residuals <- as.numeric(y - X %*% point_est)
  
  # define the optimal bandwidth
  hopt <- hopt_func(y = y,X = X,residuals_value = MODLR_residuals)
  
  # recalculate point estimation using the optimal bandwidth
  point_est <- MODLR(y = y, X = X, beta_init = beta_true, h = hopt, iter_max = 1000, tol = 1e-8)
  ...
  while (i <= B) {
    ...
    point_est_B <- tryCatch({
      MODLR(y = yB, X = XB, beta_init = point_est, h = hopt, iter_max = 1000, tol = 1e-8)
...
simu_MODLR_output <- parallel::mclapply(X = 1:300,FUN = simu_MODLR, mc.cores = 8)
